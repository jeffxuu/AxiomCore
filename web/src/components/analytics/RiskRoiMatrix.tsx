...
const RISK_FILL: Record<RiskLevel, { light: string; dark: string }> = {
  low: { light: "#0D9488", dark: "#14B8A6" },
  medium: { light: "#D97706", dark: "#F59E0B" },
  high: { light: "#E11D48", dark: "#F43F5E" },
  extreme: { light: "#9F1239", dark: "#FB7185" },
};
      ...
      <div className="relative w-full" style={{ aspectRatio: `${VIEW_W} / ${VIEW_H}` }}>
        ...
        >
          ...
          {bubbles.map(({ project, cx, cy, r }) => {
            const palette = RISK_FILL[project.risk_level];
            const isKilled = project.status === "killed";
            const opacity = isKilled ? 0.22 : project.status === "paused" ? 0.55 : 0.85;
            return (
              <g
                key={project.id}
        ...
        <div className="pointer-events-none absolute inset-0 font-sans text-[11px] tracking-tight">
          ...
          {RISK_ORDER.map((level) => {
            ...
            return (
              <span
                key={level}
                className="absolute -translate-x-1/2 uppercase tracking-[0.18em] text-[10px] text-[#64748B] dark:text-[#94A3B8]"
          ...
          {/* Quadrant tags */}
          <span
            className="absolute -translate-x-1/2 rounded-md bg-[#0D9488]/8 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#0D9488] dark:bg-[#14B8A6]/14 dark:text-[#14B8A6]"
            style={{ left: `${((PAD_LEFT + xMid) / 2 / VIEW_W) * 100}%`, top: `${(PAD_TOP - 18) / VIEW_H * 100}%` }}
          >
            {t("insights.matrix.q.star")}
          </span>
          <span
            className="absolute -translate-x-1/2 rounded-md bg-[#D97706]/8 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#D97706] dark:bg-[#F59E0B]/14 dark:text-[#F59E0B]"
            style={{ left: `${((xMid + (VIEW_W - PAD_RIGHT)) / 2 / VIEW_W) * 100}%`, top: `${(PAD_TOP - 18) / VIEW_H * 100}%` }}
          >
            {t("insights.matrix.q.speculative")}
          </span>
          <span
            className="absolute -translate-x-1/2 rounded-md bg-[#0F172A]/6 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#475569] dark:bg-[#F8FAFC]/10 dark:text-[#94A3B8]"
            style={{
          ...
          <span
            className="absolute -translate-x-1/2 rounded-md bg-[#E11D48]/8 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#E11D48] dark:bg-[#F43F5E]/14 dark:text-[#F43F5E]"
            style={{
        ...
        {hover && tooltip ? (
          <div
            className={cn(
              "pointer-events-none absolute z-20 min-w-[200px] max-w-[280px] rounded-md border bg-white px-3 py-2 shadow-lg",
              "border-[#E2E8F0] dark:border-[#1E293B] dark:bg-[#0F1B30]"
            )}
            style={{
          ...
          >
            <p className="font-sans text-[12px] font-semibold tracking-tight text-[#0F172A] dark:text-[#F8FAFC]">
              {hover.project.name}
            </p>
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 font-sans text-[11px] tracking-tight">
