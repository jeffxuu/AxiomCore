...
| 层 | 形态 | 用途 | 进 Git？ |
|---|------|------|---------|
| SQLite | `data/lifeos.db` | 运行态、查询、API 返回 | ❌ |
| Markdown frontmatter + 区块 | `02_每日记录/*.md`、`templates/*.md`、`profile/*.md` | 长期可读快照、版本回溯 | ✅ |
| JSON Schema | `data/schemas/*.schema.json` | 校验关键字段 | ✅ |

## SQLite 表结构

> 源代码权威定义在 `lifeos_server.py` 的 `init_db()` 中。下面是当前的快照。
...
1. 每次 API 写入 SQLite 后，**重写** Markdown 的 `<!-- LIFEOS:BEGIN -->` ~ `<!-- LIFEOS:END -->` 区块。
...
- 用户密码 / API 密钥 / SSH 私钥 → 见 [security.md](security.md)，存在 `.lifeos-secrets/` 或环境变量
