...
| 手动 Markdown | ⭐ 仅紧急修补 | 直接编辑 `02_每日记录/YYYY-MM-DD.md` 区块外内容 |
| 飞书富文本指令 | ⭐⭐ 周报生成 | 给机器人发"生成周报" |

**禁止：** 直接 SQL 写 SQLite。`data/lifeos.db` 应被视为应用程序私有。
...
lifeos_server.py 的 feishu_events handler
    ↓ verify token + signature
    ↓ 解析正文 (FEISHU_FIELD_PATTERNS)
    ↓ 抽取出 sleep_hours/weight_kg/mood/...
upsert SQLite daily_entries 当天行
    ↓
重写 02_每日记录/YYYY-MM-DD.md 的 LIFEOS:BEGIN/END 区块
    ↓
LifeOSCloudGitSync (云端 cron) git add+commit+push origin main
    ↓
...
字段抽取由 `FEISHU_FIELD_PATTERNS` 字典驱动（在 [lifeos_server.py](../lifeos_server.py) 顶部）。支持的字段：
...
浏览器 5173/8765
    ↓ POST /api/days/{date}
lifeos_server.py 的 save_day handler
    ↓ Pydantic 校验 (LifeOSEntry + LifeOSTask)
upsert SQLite (daily_entries, daily_tasks)
    ↓
重写 02_每日记录/YYYY-MM-DD.md LIFEOS:BEGIN/END
```
...
```
本地 Windows 计划任务 LifeOSCloudPullSync
    ↓ 每 5 分钟
scripts/cloud_to_local_sync.ps1
    ↓ SSH 拉取云端 02_每日记录/*.md
    ↓ 解析 LIFEOS:BEGIN/END 区块字段
    ↓ 写入本地 SQLite + 重写本地 Markdown
```

凭据来自 `scripts/lifeos_secrets.ps1` DPAPI 加密存储，**不在仓库内**。
...
由于本地与云端**双向写**同一份 02_每日记录，理论存在冲突。实际策略：

1. **同一天的字段冲突**：以时间靠后的 upsert 为准（SQLite 写入顺序）。
2. **不同分支 git 冲突**：人工 `git pull --rebase` + 手动解决。这种情况罕见，因为两端写入的都是 LIFEOS 区块内同样的字段集。
...
workflows/summarize_fragments.py
    ...
    ↓ 写入 profile/current-state.md 的 LIFEOS:BEGIN/END 区块
...
- ❌ **绕过 Markdown 区块直接编辑 LIFEOS:BEGIN/END 之间的内容**：下次 upsert 时被覆盖。
